...
FLASHMEM ILI948x_t41_p::ILI948x_t41_p(const uint8_t * init_commands, int8_t dc, int8_t cs, int8_t rst) 
{
  
  _dc = dc;
  _cs = cs;
  _rst = rst;
  _init_commands = (uint8_t*)init_commands;
...
FASTRUN void ILI948x_t41_p::displayInfo(){
  CSLow();
...
FASTRUN uint8_t ILI948x_t41_p::readCommand(const uint8_t cmd){
  while(WR_IRQTransferDone == false)
  {
    //Wait for any DMA transfers to complete
  }
